...
public interface PassgaeRespository extends JpaRepository<PassgaeEntity,Integer> {
    ...
    @Transactional
    @Query(value = "select p.name,p.content,p.time from passgae p where c_id=?1 order by time desc",nativeQuery = true)
    List<Object[]> getAllPas(int cid);

    //方式二：
    //根据社长给输入内容进行模糊查询
    @Transactional
    @Query(value = "select p.name,p.content,p.time from passgae p where c_id=?1 and content like ?2",nativeQuery = true)
